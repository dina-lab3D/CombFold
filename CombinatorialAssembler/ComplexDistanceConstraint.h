...
public:
  ...
  float getRatio(const std::vector <std::shared_ptr<const BB>>& bbs,
    ...
    float ratio = 1.0;
    if(total > 0) ratio = (float)count/total;//(2*numberOfRestraints_);
    return ratio;
  }
  ...
  bool isSatisfied(const std::vector <std::shared_ptr<const BB>>& bbs,
                   const std::vector<RigidTrans3> &trans) const {
...
private:
  ...
  void addConstraint(int suInd1, int suInd2, Vector3 receptorAtom, Vector3 ligandAtom, float maxDistance, float minDistance  = 0);

  void addRestraint(int suInd1, int suInd2, Vector3 receptorAtom, Vector3 ligandAtom, float maxDistance, float minDistance  = 0);
