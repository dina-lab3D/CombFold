...
class TransformationAndScore_T {
public:
...
template <class SCORE_T>
std::ostream& operator<<(std::ostream& s, const TransformationAndScore_T<SCORE_T> &ts) {
  return s << ts.refFrame_ << " " << ts.score_ << " , " << ts.dist_<< " , " << ((ts.dist_ == 0) ? 1 : 0);
}

template <class SCORE_T>
std::istream& operator>>(std::istream& s, TransformationAndScore_T<SCORE_T> &ts) {
  char c;
