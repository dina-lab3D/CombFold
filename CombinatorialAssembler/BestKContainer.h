...
public:
  ...
  bool isEmpty(const unsigned long set) const {
    return (set2index_.find(set) == set2index_.end());
  }
  ...
  BestK& operator[] (const unsigned long set) {
    return *((std::vector<BestK*>)(*this))[set2index_[set]];
  }
