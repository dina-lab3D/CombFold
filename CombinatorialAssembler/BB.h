...
class InterestPoint {
public:
  InterestPoint(Vector3 position, Vector3 probePosition, bool hydrophobic) : position_(position), probePosition_(probePosition), hydrophobic_(hydrophobic) {}
  Vector3 position_;
...
class BBConstructor{
    // idea taken from https://stackoverflow.com/questions/3220009/is-this-key-oriented-access-protection-pattern-a-known-idiom
private:
...
public:
  ...
  void putTransWith(int bbIndex,
                    const std::shared_ptr<TransformationAndScore>& t1,
                    const BBConstructor&) const { trans_[bbIndex].push_back(t1);}
