...
void HierarchicalFold::createSymmetry(std::vector <std::shared_ptr<SuperBB>> identBBs, BestK &results) {
    ...
    for (TransIterator2 it(*identBBs[0], *identBBs[1], identBBs[0]->bbs_[0]->getID(), identBBs[1]->bbs_[0]->getID()); !it.isAtEnd(); it++)
        transCount++;
    ...
    for(unsigned int transNum=0; transNum < transCount; transNum++) {
        ...
        for(unsigned int i=1; i < identBBs.size(); i++) {
            ...
            for (TransIterator2 it(*symSBB, *identBBs[i], symSBB->bbs_[i-1]->getID(), identBBs[i]->bbs_[0]->getID()); !it.isAtEnd(); it++) {
                count++;
                if(count != transNum)
                    continue;
...
//    for (TransIterator2 it(*identBBs[0], *identBBs[1], identBBs[0]->bbs_[0]->getID(), identBBs[1]->bbs_[0]->getID()); !it.isAtEnd(); it++) {
//        std::cout << "started trans check, bb_size:" << identBBs.size() << " trans " << it.transformation() << std::endl;
//
//
...
//        }
        ...
        if(bbPenetrations / (1.0 * totalUsedAtoms) > 0.2){
            std::cout << "dropping " << identBBs.size() << " because penetration " << bbPenetrations / (1.0 * totalUsedAtoms) << std::endl;
            continue;
        }
        ...
        if((centroids[0] - centroids[1]).norm2() * allowedDistFactor < (centroids[0] - centroids.back()).norm2()){
            std::cout << "dropping " << identBBs.size() << " because centroids " << (centroids[0] - centroids[1]).norm2() << " : " << (centroids[0] - centroids.back()).norm2() << std::endl;
            continue;
        }
...
void HierarchicalFold::fold(const std::string& outFileNamePrefix) {
    ...
    for (std::vector<unsigned int> identGroup: identGroups){
        ...
        if(groupSBBs.size() >= 3 && groupSBBs.size() == N_){
            std::cout << "searching for size " << groupSBBs.size() << " has " << keptResultsByLength.count(groupSBBs.size()) << std::endl;
    ...
    for (unsigned int length = 2; length <= N_; length++) { // # subunits iteration
        std::cout << "*** running iteration " << length << " prev kept results: " <<  keptResultsByLength[length - 1]->size() << std::endl;
        ...
        for (auto it1 = newKept->begin(); it1 != newKept->end(); it1++){ 
            ...
            if (best_k_by_id.count(currResSet) == 0) best_k_by_id[currResSet] = new BestK(K_);
	...
	for(unsigned int firstResultSize = 1; firstResultSize <= length / 2; firstResultSize++) {
            ...
            for (auto it1 = keptResultsByLength[firstResultSize]->begin(); it1 != keptResultsByLength[firstResultSize]->end(); it1++) {
                unsigned long setA = (**it1).bitIds();
                for (auto it2 = keptResultsByLength[secondResultSize]->begin(); it2 != keptResultsByLength[secondResultSize]->end(); it2++) {
                    unsigned long setB = (**it2).bitIds();
                    if ((setA & setB) != 0) continue;
                    if (firstResultSize == secondResultSize && setA < setB) continue;  // Since those are 2 identical loops, don't do things twice

                    unsigned long currResSet = setA | setB;

                     if(!isValidBasedOnIdent(identGroups, currResSet)) continue;
                    ...
                    this->tryToConnect(1, **it1, **it2, *best_k_by_id[currResSet], (length < N_), promise1, identGroups);
        ...
        for (const auto& [currResSet, currBestK] : best_k_by_id) {
		    ...
		    //    clusteredBestK->insert(*it);
                ...
                std::cerr << "clustering resSet " << currResSet << " before: " << currBestK->size()
                          << " after " << clusteredSBBS_[currResSet].size() << std::endl;
        ...
        for (auto it1 = newKept->begin(); it1 != newKept->end(); it1++){
            if (count_new_kept_by_resSet.count((**it1).bitIds()) == 0) count_new_kept_by_resSet[(**it1).bitIds()] = 0;
        ...
        for(const auto& elem : count_new_kept_by_bb) std::cout << elem.first << ":" << elem.second << ", ";
        std::cout << std::endl << "new kept results by resSet ";
        for(const auto& elem : count_new_kept_by_resSet) std::cout << elem.first << ":" << elem.second << ", ";
        std::cout << std::endl;
    }

  outFile.close();

  if(!output) outputSubsets();
}
  ...
  for (unsigned int length = N_-1; length > 2; length--) { // # subunits iteration
    ...
    do { // iterate subsets of length out of N
      ...
      if(!clusteredSBBS_.isEmpty(currResSet)) {
        found = true;
        std::string outFileName = "cb_" + std::to_string(length) + "_"  +
          std::to_string(currResSet) + ".res";
...
void HierarchicalFold::tryToConnect(int id, const SuperBB &sbb1, const SuperBB &sbb2,
...
//        }
        ...
        bool filtered = filterTrans(sbb1, sbb2, it.transformation(),
                                      penetrationThreshold_, bbPen,
                                      backbonePenetrationThreshold, newScore,
                                      i, j);
        if (filtered) continue;

        int numBurried = 0; //numBurried2;// + numBurried1;
        FoldStep step(firstBB, secondBB, it.getScore());
        std::shared_ptr<SuperBB> theNew = createJoined(sbb1, sbb2, it.transformation(),
                                                       numBurried, bbPen, newScore, step,
                                                       it.getScore());

        if(theNew->getRestraintsRatio() < complexConst_.getDistanceRestraintsRatio()){
//            std::cout << "not enough restraints " << theNew->getRestraintsRatio() << " : " << complexConst_.getDistanceRestraintsRatio();
            continue;
        }
...
bool HierarchicalFold::filterTrans(const SuperBB &sbb1, const SuperBB &sbb2, const RigidTrans3 &trans,
  ...
  for (unsigned int i = 0; i < sbb1.size_; i++) {
    ...
    for (unsigned int j = 0; j < sbb2.size_; j++) {
      ...
      if (!complexConst_.isSatisfied(bb1.getID(), bb2.getID(), t2)) return true;
      // check restraints
...
//                          // std::cout << " bbPenetrations " << bbPenetrations << " " << maxBBPenetrations;
//                          return true;
//                      }
                  }
              }
